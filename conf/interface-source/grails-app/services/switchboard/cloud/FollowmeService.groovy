...
class FollowmeService {
    ...
    boolean createFollowme(Long agentId){
        boolean success = true
        Agent agent = Agent.get(agentId)
        FollowMeConfig fmConfig = new FollowMeConfig(name: agent.id)

        try {
            fmConfig.save(failOnError: true)
        }
        catch(Exception e){
            success = false
            log.error "Failed creating Followme Config for $agent.name: ${e.message}"
