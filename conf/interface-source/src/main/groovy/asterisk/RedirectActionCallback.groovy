...
class RedirectActionCallback implements SendActionCallback{
    def log

    RedirectActionCallback(logger){
        log = logger
    }

    void onResponse(ManagerResponse response){
        log.debug "RedirectActionCallback"
        response.properties.each { key, value ->
            log.debug "$key: $value"
        }
    }
}
